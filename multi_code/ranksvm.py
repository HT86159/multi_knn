...
from tree.ml_dt import MLDecisionTree
# from mlclas.neural import BPMLL
from svm.ranking_svm import RankingSVM
...
# # print(pca.transform(test_data.todense()))
# # print(type(pca.transform(test_data.todense())))
...
#     i = 0
#     while(i < len(data_y)):
#         if (data_y[i] == full_true).all() or (data_y[i] == full_false).all() or sum(data_y[i])==0:
...
# def data_hand(data_set):
#     tempt = []
#     for j in range(len(data_set)):
#         for i in range(len(data_set[j])):
#             tempt.append([j,data_set[j][i]])
#     return tempt
#
#
...
# # print(x_train)
# # print(y_train)
# result,result1,result2 = RankingSVM(normalize='fs', print_procedure=True).fit(x_train, y_train, 0.05).predict(x_test)
# #result1,result2 = RankingSVM(normalize='l2', print_procedure=True).fit(train_data_trans, train_target, 8).predict(test_data_trans)
# # for i in range(10):
# c = 0
# y_test2 = []
...
#     s = set(np.where(test_target[i]==1)[0])
#     print(s)
#     if result2[i] in s:
#         c+=1
# print(c)
...
print(y_train)
result,result1,result2 = RankingSVM(normalize='fs', print_procedure=True).fit(x_train, y_train, 0.05).predict(x_test)
scio.savemat("ranksvm_result.mat",{"result":result})
