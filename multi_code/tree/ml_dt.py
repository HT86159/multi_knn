from multi_code.utils.check import check_feature_input
import multi_code.tree.dt_models as dtm
...
class MLDecisionTree:
    ...
    def predict(self, x):
        ...
        result2 = []
        result3 = []
        for index in range(samples):#对输入的每一个数据进行预测
            values = x[index]
            treenode = self.root
            ...
            r1,r2 = treenode.get_prediected_labels()
            result1.append(r1)
            result2.append(np.argmax(r2))
            result3.append(copy.deepcopy(r2))
